...
- [~] **P2 · Partial: emulated SD + SMB + storage/commands scaffolded; SD-present status deployed and Pi-side live-verified; app-side Connect confirmation pending**
...
- **Live (Pi-side) 2026-09-19:** deployed via `deploy.sh` (overlay maintenance flow, services `active`); `/mnt/sdcard` is owned by the service user and writable; the deployed `timelapse.storage_status()` returns `(1, <total>, <free>, <used>, 'RW')` (present, RW); `smbd` active; `/c/info` 200 and `status` sent (387 bytes). App-side confirmation that Connect stops reporting "storage not detected" is still pending.
- **Still open:** live Connect confirmation that the app stops reporting "storage not detected"; the exact file-list envelope annotation; progress `client_trigger` (`GAP-SIO-01`).
