...
- **Live (app-side) confirmed 2026-09-19:** after the deploy, Connect shows timelapse **available**; the storage page displays SD size/used/free and the interval is configurable. No frame capture / make-video / file-list round-trip has been exercised yet.
- **Still open:** confirming that the app's enable/interval actually reach the Pi (the `configuration` tag3 timelapse subfields are not yet mapped — GAP-CONFIG-01) and that frames record; the exact file-list envelope annotation; progress `client_trigger` (`GAP-SIO-01`).
