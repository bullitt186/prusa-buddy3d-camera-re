...
- **Recovered 2026-09-19 (live):** the full camera-side flow is implemented — ICE config → camera **offer** (type 3) → viewer **answer** (type 2) → **trickle candidates** (type 4). The viewer's candidates arrive as `a=candidate:...` with `tag2 = mid` and are now applied via `add-ice-candidate` (they were previously dropped, which is why the connection never completed). Offer/answer reach the viewer.
- **Remaining blocker (verified live 2026-09-19):** the Connect answerer (itself a **libdatachannel** endpoint) rejects our H264 m-line (`m=video 0`). It **validates the actual SPS**: setting the SDP `a=fmtp` to libdatachannel's `profile-level-id=42e01f` while the stream SPS is `428029` makes it stop answering entirely; the unmodified SPS yields the `m=video 0` rejection. Our v4l2/rpicam-vid encoder emits `428029` (baseline, constraint `0x80`, level 4.1) and **rejects `--level 3.1`**, whereas the firmware's libdatachannel offer is `42e01f` (constrained baseline, constraint `0xe0`, level 3.1).
- **Transcode attempts (2026-09-19, live):** (a) `openh264enc` with `video/x-h264,profile=constrained-baseline` produces the right SPS (`42c01f`) but the software decode+encode drove the Pi (415 MB RAM) to load ~12 and made it unreachable — reverted (`d4a4c3d`); (b) `v4l2h264enc` exposes `profile=constrained-baseline`/`level=3.1` via **src caps** (not properties) but fails at runtime ("Failed to process frame" — the hardware encoder is owned by the camera source). Remaining option: rewrite the SPS profile/level bytes on the WebRTC branch (no transcode), or a lighter re-encode.
