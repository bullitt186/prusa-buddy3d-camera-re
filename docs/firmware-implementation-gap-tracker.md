...
- [~] **P2 · Partial: emulated SD + SMB + storage/commands scaffolded; SD-present status not yet wired**
- **2026-09-19 (live):** Connect shows *"Time lapse not available, camera storage not detected, insert SD card"*. The app reads storage from the **`status` message**, specifically the `extended_status.4` storage block (descriptor `0x3f72b0`; firmware labels it the "video/timelapse mode/storage block"). We emit `{1:2, 2:0, 3:0, 4:0, 5:MODEL}`; the firmware emits `{1:1, 2:FUN_000abcb0(), 3:FUN_000abaf4(), 4:<uvarint>, 5:<string>}`, where the two getters read a shared singleton (`+0x4`, `+0x50`) that is populated from `/sys/class/block/mmcblk{0,1}` (strings at `lp_app.strings:1684-1685`, `getSdCardSpace` at `:11874`). **Next step:** trace that singleton (or set the block to the firmware shape) to report SD-present + free space; do not guess the numeric "present" value.
...
- **Still open:** SD-present/free-space in `extended_status.4`; the exact file-list envelope annotation; progress `client_trigger` (`GAP-SIO-01`).
